...
def simulate_n_trajectories(
  ...
  Args:
    ...
    goal: goal vectors [num_simulations, G]
    rng: random key
  ...
  def get_q_vals(lstm_out, w):
    main_q = network.apply(params, lstm_out, w, method=network.main_task_q_fn)
    offtask_q = network.apply(params, lstm_out, w, method=network.off_task_q_fn)
  ...
  def initial_predictions(x, prior_h, w, rng_):
    ...
    preds = Predictions(q_vals=get_q_vals(lstm_out, w), state=lstm_state)
  ...
  def _single_model_step(carry, inputs):
    ...
    next_preds = Predictions(
      q_vals=jax.vmap(get_q_vals)(next_rnn_out, goal), state=next_lstm_state
    )
...
class DuellingDotMLP(nn.Module):
  ...
  def __call__(self, x, task, train: bool = False):
    ...
    advantages = jnp.reshape(advantages, advantages_shape)
    advantages -= jnp.mean(advantages, axis=-2, keepdims=True)

    sf = value + advantages
    q_values = jnp.sum(sf * jnp.expand_dims(task, axis=-2), axis=-1)
    return q_values
...
class PreplayLossFn:
      ...
      def all_goals_loss_single(
        ...
        def apply_q(params_, rnn_out_rep, q_method):
          ...
          Returns:
            [T, N, A] Q-values
          """
          q_fn = lambda r, g: self.network.apply(params_, r, g, method=q_method)
  ...
  def preplay_loss_fn(
    ...
    def preplay_at_t(x_t, h_on_t, h_tar_t, l_mask_t, key):
      ...
      # Single simulate call (uses online params for RNN + action selection)
      ...
      all_a = sim_out.actions  # [S+1, total_sims]
      # Online RNN outputs (from simulation with online params)
      online_rnn_out = rnn_output_from_state(
      ...
      all_mask = simulation_finished_mask(init_mask, all_t)  # [S+1, total_sims]

      # === MAIN-TASK LOSS on ALL sims ===
      all_main_goal = jnp.tile(main_goal, (total_sims, 1))  # [total_sims, G]
      ...
      off_g = all_goals[N_on:]  # [G_off*N_off, G]
...
def make_train_craftax_singlegoal(**kwargs):
  ...
  def dyna_epsilon_values(rng):
    return jnp.zeros(num_ontask_simulations)

  def offtask_dyna_epsilon_values(rng):
    epsilons = jax.random.choice(rng, vals, shape=(num_offtask_simulations - 1,))
...
def make_train_craftax_multigoal(**kwargs):
  ...
  def sample_nontask_visible_goals(timestep, key, num_offtask_goals):
    is_object_nearby = timestep.observation.nearby_objects.astype(jnp.int32)
    is_object_nearby = is_object_nearby.sum(-1)
    task_object = timestep.observation.task_w.astype(jnp.int32)
  ...
  def get_main_goal(timestep):
    ...
    return jnp.tile(goal, (num_ontask_simulations, 1))
...
def learner_log_extra(
  ...
  def log_data(
    ...
    **kwargs,
    ...
    actions_ep = actions[in_episode][:-1]
    ...
    renderer.place_arrows_on_image(
      img, positions, actions_ep, maze_height, maze_width, arrow_scale=5, ax=ax
    )
    if wandb.run is not None:
