...
def simulate_n_trajectories(
    ...
    sim_outputs = SimulationOutput(
        predictions=concat_first_rest(
            preds_t, sim_outputs.predictions),
...
class DynaLossFn(vbb.RecurrentLossFn):
  ...
  def error(self,
    ...
    **kwargs):
    ...
    if self.dyna_coeff > 0.0:
        ...
        h_tm1_online = concat_first_rest(
            online_state, remove_last(online_preds.state)
        )
        h_tm1_target = concat_first_rest(
            target_state, remove_last(target_preds.state)
        )
        x_t = data.timestep

        dyna_loss_fn = functools.partial(
            self.preplay_loss_fn,
            params=params,
        ...
        all_metrics.update(
            {f'{k}/dyna': v for k, v in dyna_metrics.items()})
  ...
  def preplay_loss_fn(
    ...
    def offtask_dyna_loss_fn_(
      ...
      entropy = -jnp.sum(achieve_poss * jnp.log(achieve_poss + 1e-5), axis=-1) / jnp.log(achieve_poss.shape[-1])
      ...
      offtask_metrics['2.num_achievable'] = all_t.observation.achievable.sum(-1)  # unnormalized
      ...
      # Get Main task loss
      #########################################
      main_batch_td_error, main_batch_loss_mean, main_metrics, main_log_info = self.loss_fn(
          timestep=all_t,
          online_preds=online_preds,
          target_preds=target_preds,
          actions=all_a,
...
class DynaAgentEnvModel(nn.Module):
    ...
    def apply_model(
        ...
        return jax.vmap(env_step)(
            state, action, jax.random.split(rng, B))
