...
def create_metrics(log_states, dones, prefix="eval"):
  ...
  metrics[f"{main_key}/0.score"] = (
    log_states.returned_episode_returns / MAX_SCORE
  ) * 100.0
  ...
  metrics = jax.tree.map(lambda x: (x * dones).sum() / (1e-5 + dones.sum()), metrics)
  return metrics
...
def evaluate_model(network, params, rng, env, test_env_params, config):
  ...
  def _eval_step(carry, unused):
    ...
    # Step environment
    rng, _rng = jax.random.split(rng)
    next_obs, log_state, reward, done, info = env.step(
      _rng, log_state, action, test_env_params
    )
