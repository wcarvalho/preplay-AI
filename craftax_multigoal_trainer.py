  ...
  **kwargs,
  ...
  def callback(m, n_logs, env_state, done_flags, task_w, rewards):
    ...
    if (
      ...
      max_episodes = min(8, len(ep_starts))
      if max_episodes == 0:
        return

      ncols = min(4, max_episodes)
      nrows = (max_episodes + ncols - 1) // ncols
      ...
      for ep_idx in range(max_episodes):
        ...
        # Render initial state of this episode
        ep_env_state = jax.tree_util.tree_map(lambda x: x[start], env_state)
        img = render_state(ep_env_state)
        ax.imshow(img)
        ...
        ep_len = end - start
        ax.set_title(f"{goal_name}\nr={ep_reward:.1f}, len={ep_len}", fontsize=10)
        ax.axis("off")

      for i in range(max_episodes, len(axes)):
        axes[i].axis("off")
