...
def load_env_params(
  ...
  def make_reset_params(map_init, train_objects, test_objects, **kwargs):
  ...
  for idx, goal in enumerate(train_objects):
    path = jaxmaze_utils.find_optimal_path(
  ...
  if alg_name == "qlearning":
    make_train = functools.partial(
      ...
      make_agent=functools.partial(
        ...
        ObsEncoderCls=networks.JaxmazeObsEncoder,
      ),
    ...
    import distrax
    import train_extra_replay

    sim_policy = config.get("SIM_POLICY", "gamma")
    num_simulations = config.get("NUM_SIMULATIONS", 15)
    if sim_policy == "gamma":
      temp_dist = distrax.Gamma(
        concentration=config.get("TEMP_CONCENTRATION", 1.0),
        rate=config.get("TEMP_RATE", 1.0),
      )

      rng, rng_ = jax.random.split(rng)
      temperatures = temp_dist.sample(seed=rng_, sample_shape=(num_simulations,))
      ...
      epsilons = jax.random.choice(rng, vals, shape=(num_simulations,))
