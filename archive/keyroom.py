...
# from minigrid_common import take_action as take_action_new
...
def swap_test_pairs(maze_config: dict):
  maze_config = copy.deepcopy(maze_config)
  maze_config["pairs"][0][1], maze_config["pairs"][1][1] = (
    maze_config["pairs"][1][1],
    maze_config["pairs"][0][1],
  )
  try:
    maze_config["pairs"][2][1], maze_config["pairs"][3][1] = (
      maze_config["pairs"][3][1],
      maze_config["pairs"][2][1],
    )
  except:
    pass
  return maze_config
...
class TaskRunner(struct.PyTreeNode):
  ...
  def get_features(self, visible_grid: GridState, agent: AgentState):
    ...
    pocket = make_task_obj(
      *agent.pocket, visible=1, state=States.PICKED_UP, asarray=True
    )
  ...
  def reset(self, visible_grid: GridState, agent: AgentState):
    ...
    return TaskState(feature_counts=features, features=features)
  ...
  def step(self, prior_state: TaskState, visible_grid: GridState, agent: AgentState):
    ...
    return TaskState(feature_counts=feature_counts, features=new_features)
...
class KeyRoomEnvParams(EnvParams):
  ...
  train_multi_probs: float = struct.field(pytree_node=False, default=0.5)
...
def get_room_grid(grid: GridState, agent: AgentState):
  ...
  return jax.lax.dynamic_slice(grid, (start_y, start_x, 0), (delta, delta, grid_dim))
...
def place_in_room(y, x, rng, grid, obj: tuple, off_border=True, roomW=None, roomH=None):
  assert roomW is not None and roomH is not None
  coords, rng = sample_coordinates(
    y=y, x=x, rng=rng, grid=grid, roomW=roomW, roomH=roomH
  )
  grid = grid.at[coords[0], coords[1]].set(obj)
  return grid, rng


def fixed_place_in_room(
  y, x, rng, grid, obj: tuple, idx: int, off_border=True, roomW=None, roomH=None
):
  assert roomW is not None and roomH is not None
  xL, yT, xR, yB = get_x_y(y=y, x=x, roomW=roomW, roomH=roomH)
...
def render_room(state: EnvState, render_mode: str = "rgb_array", **kwargs):
  ...
  localized_agent = state.agent.replace(position=local_agent_position)
...
def prepare_task_variables(
  ...
  for room_idx in range(n_task_rooms):
    ...
    train_w.append((key_reward, door_reward, 1.0, 0))
    test_w.append((0.0, 0.0, 0.0, 1.0))
...
class KeyRoom(Environment[KeyRoomEnvParams, EnvCarry]):
  ...
  def default_params(
    self, maze_config: dict, height=19, width=19, **kwargs
  ) -> KeyRoomEnvParams:
    maze_config = convert_dict_to_types(maze_config)

    task_objects, train_w, test_w = prepare_task_variables(maze_config)
  ...
  def _generate_problem(
    self, params: KeyRoomEnvParams, rng: jax.Array
  ) -> State[EnvCarry]:
  ...
  def singleroom_problem(
    ...
    goal_room_idx = jax.random.randint(rng, shape=(), minval=0, maxval=len(pairs))
    pair = index(pairs, goal_room_idx)
    if params.random_obj_loc:
      grid, rng = place_in_room_p(1, 1, rng, grid, make_obj(*pair[0], visible=1))
      grid, rng = place_in_room_p(1, 1, rng, grid, make_obj(*pair[1], visible=1))
    else:
    ...
    state = EnvState(
      ...
      grid=grid,
      agent=agent,
      local_agent_position=get_local_agent_position(agent.position, *grid.shape[:2]),
  ...
  def multiroom_problem(
    ...
    if params.random_door_loc:
      door_coords = jax.random.randint(rngs[0], shape=(nrooms,), minval=1, maxval=roomW)
    else:
    ...
    add_door_fns = [add_right_door, add_bottom_door, add_left_door, add_top_door]
    ...
    for room_idx in range(nrooms):
      ...
      if params.random_obj_loc:
        grid, rng = place_in_room_p(
          1, 1, rng, grid, make_obj(*keys[room_idx], visible=1)
        )
      else:
        xL, yT, _, _ = get_x_y(y=1, x=1, roomW=roomW, roomH=roomH)
      ...
      door = make_obj(Tiles.DOOR_LOCKED, door_color, visible=1)
      ...
      obj1, obj2 = pairs[room_idx]
      if params.random_obj_loc:
        grid, rng = place_in_room_p(*obj_coords, rng, grid, make_obj(*obj1))

        grid, rng = place_in_room_p(*obj_coords, rng, grid, make_obj(*obj2))
      else:
        grid, rng = fixed_place_in_room(
          *obj_coords,
          rng,
    ...
    def get_train_or_test(rng):
      ...
      return jax.lax.cond(get_train, get_train_object, get_test_object, rng)
  ...
  def single_room_reward_termination(
    self, params, state: EnvState, observation: Observation
  ):
    def reward_fn(state: EnvState, observation: Observation):
      state_features = observation.state_features.astype(jnp.float32)
  ...
  def multi_room_reward_termination(
    self, params, state: EnvState, observation: Observation
  ):
    def reward_fn_train(s: EnvState, o: Observation):
      state_features = o.state_features.astype(jnp.float32)
    ...
    def reward_fn(state: EnvState, observation: Observation):
      return jax.lax.cond(
        params.training, reward_fn_train, reward_fn_test, state, observation
      )

    def termination_fn(state: EnvState, observation: Observation):
      return jax.lax.cond(
        params.training, termination_fn_train, termination_fn_test, state, observation
      )
  ...
  def reset(self, key: jax.random.PRNGKey, params: EnvParamsT) -> TimeStep[EnvCarryT]:
    state = self._generate_problem(params, key)

    task_state = self.task_runner.reset(visible_grid=state.room_grid, agent=state.agent)
  ...
  def teleport_agent_remove_key_close_door(self, prior_timestep, new_grid, new_agent):
    def move(position: jax.Array, direction: jax.Array, steps: int = 2) -> jax.Array:
    ...
    def teleport_agent_remove_key_close_door(grid, agent, door_opened_somewhere):
      # get door identity
      y, x = jnp.nonzero(door_opened_somewhere, size=1)
      door = grid[y[0], x[0]]  # [D=2]

      # get which task object door corresponded to
      # [num_tasks, num_task_objects]
      which_task_obj_is_door = (
        door[None, None] == self.task_runner.task_objects[:, :, :2]
      ).all(-1)

      # [num_tasks, num_task_objects]
      feature_counts = prior_timestep.state.task_state.feature_counts
      feature_counts_door = (
        which_task_obj_is_door.astype(jnp.float32) * feature_counts
      ).sum()

      def update_agent_grid(g, a):
        position = move(a.position, a.direction)
        new_a = a.replace(
          position=position, pocket=TILES_REGISTRY[Tiles.EMPTY, Colors.EMPTY]
        )

        new_g = g.at[y[0], x[0]].set(TILES_REGISTRY[Tiles.DOOR_CLOSED, door[COLOR_IDX]])
    ...
    # door opened?
    # --------
    door_opened_somewhere = Tiles.DOOR_OPEN == new_grid[:, :, 0]
  ...
  def take_action(
    ...
    new_grid, new_agent = self.teleport_agent_remove_key_close_door(
      prior_timestep=timestep, new_grid=new_grid, new_agent=new_agent
    )
  ...
  def step(
    ...
    new_task_state = self.task_runner.step(
      prior_state=timestep.state.task_state, visible_grid=new_room_grid, agent=new_agent
    )
    ...
    new_observation = make_observation(
      new_state, prev_action=self.action_onehot(action, params), params=params
    )
