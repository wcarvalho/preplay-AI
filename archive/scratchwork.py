...
for idx, (manipulation, model_setting, title, fn) in enumerate(
  zip(manipulations, model_settings, titles, fns)
):
  ...
  data = dict(
    ...
    human_success=get_human_data(
      fn=fn,
      filter_fn=lambda e: housemaze_analysis.success(e) < 1,
      manipulation=manipulation,
      eval=True,
    ),
    human_failure=get_human_data(
      fn=fn,
      filter_fn=lambda e: housemaze_analysis.success(e),
      manipulation=manipulation,
      eval=True,
    ),
    human_terminate=get_human_data(
      fn=fn,
  ...
  housemaze_analysis.bar_plot_results(data, title=title, ylabel="Proportion")
  ...
  housemaze_analysis.bar_plot_results(
    data, title=f"Manipulation {manipulation}: Success rate", ylabel="Rate"
  )
  #####################
  # Termination rate
  #####################
  ...
  # pprint(reused_path_data)
  housemaze_analysis.bar_plot_results(
    data, title=f"Manipulation {manipulation}: Termination rate", ylabel="Rate"
  )
